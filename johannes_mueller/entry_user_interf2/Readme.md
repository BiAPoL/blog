...
Create a main window by clicking selecting "Main Window" from the list and then ```Create```. This brings up a blank window which you can now populate with widgets. Let's go through the elements!
...
```python
# Plot a curve:
plotWidget.canvas.axes.plot(x_data, y_data, color='red')
